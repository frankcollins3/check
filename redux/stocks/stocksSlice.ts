  ...
  import { candlestickINTERFACE, companyINTERFACE, candlestickARRAYTYPE } from 'Interface/InterfaceTypes';
  ...
  interface StocksSliceState {
    ...
    ANALYST_PAGINATION_INDEX_1: number;
    ANALYST_PAGINATION_INDEX_2: number;
    ANALYST_PAGINATION_INDEX_3: number;
    MORE_ANALYST_CLICK_INDEX: number

    NAVBAR_INPUT_IS_HOVERED_ON: boolean;
    NAVBAR_INPUT_IS_FOCUSED_ON_YET: boolean;
    NAVBAR_INPUT_VALUE: string
  ...
  const initialState: StocksSliceState = {    
    CURR_COMPANY: {
      company: '',
      ticker: '',
      // object was originally empty. now providing data so app can also go direction of importing other data         
      keyRatios: {
          company: '',
          ticker: '',
          id: 0,
    ...
    APPLE:{
      // name, founders, headquarters, current_ceo, directors could go on and on but no need.
        company: 'Apple',
        ticker: 'AAPL',
        keyRatios: {
            id: 0,
    ...
    CANDLESTICK_CHART_CURR_DATA: [{ id: null, company: '', ticker: '', open: 0, close: 0, high: 0, low: 0, volume: 0, date: '', logo: '' }],
    ...
    ANALYST_PAGINATION_INDEX_1: 0,
    ANALYST_PAGINATION_INDEX_2: 1,
    ANALYST_PAGINATION_INDEX_3: 2,
    MORE_ANALYST_CLICK_INDEX: 0,

    NAVBAR_INPUT_IS_HOVERED_ON: false,
    NAVBAR_INPUT_IS_FOCUSED_ON_YET: false,
    NAVBAR_INPUT_VALUE: "what are you checking for..."
  ...
  const stocksSlice = createSlice({
    ...
    CLEAR_CANDLESTICK_CHART_FILTER: (state) => { state.CANDLESTICK_CHART_FILTER_START_DATE = '', state.CANDLESTICK_CHART_FILTER_END_DATE = '' },      
      ...
      SET_ANALYST_PAGINATION_INDEX_1: (state, action) => { state.ANALYST_PAGINATION_INDEX_1 = action.payload },
      SET_ANALYST_PAGINATION_INDEX_2: (state, action) => { state.ANALYST_PAGINATION_INDEX_2 = action.payload },
      SET_ANALYST_PAGINATION_INDEX_3: (state, action) => { state.ANALYST_PAGINATION_INDEX_3 = action.payload },
      SET_MORE_ANALYST_CLICK_INDEX: (state, action) => { state.MORE_ANALYST_CLICK_INDEX = action.payload },
      
      // NAVBAR_INPUT_IS_HOVERED_ON_YET: false,
      // NAVBAR_INPUT_VALUE: "what are you checking for..."
      SET_NAVBAR_INPUT_VALUE: (state, action) => { state.NAVBAR_INPUT_VALUE = action.payload },
      TOGGLE_NAVBAR_INPUT_IS_HOVERED_ON: (state) => { state.NAVBAR_INPUT_IS_HOVERED_ON = !state.NAVBAR_INPUT_IS_HOVERED_ON },
      TOGGLE_NAVBAR_INPUT_IS_FOCUSED_ON_YET: (state) => { state.NAVBAR_INPUT_IS_FOCUSED_ON_YET = !state.NAVBAR_INPUT_IS_FOCUSED_ON_YET },
  ...
  { 
      ...
      SET_NAVBAR_INPUT_VALUE, TOGGLE_NAVBAR_INPUT_IS_HOVERED_ON, TOGGLE_NAVBAR_INPUT_IS_FOCUSED_ON_YET
